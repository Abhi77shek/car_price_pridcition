...
class CarPageClass:
    def __init__(self,home_window):
        ...
        self.l8 = Label(self.window, text=" - - - - - ",font=('Century', 14, 'bold'),foreground=dp.textcolor,background=dp.pagebkcolor)
        ...
        self.v1 = StringVar()
        self.c1 = Combobox(self.window, values=["No","Second Hand"], textvariable=self.v1, state='readonly',font=dp.textfont,foreground=dp.textcolor)
        self.v1.set("0")
    ...
    def saveData(self):
        ...
        try:
            ...
            rowcount = self.curr.execute(qry, (int(self.t1.get()),str(float(self.t2.get())) ,str(float(self.t3.get())),
                                               self.v1.get(), self.v2.get(), self.v3.get(), self.v4.get(),self.output,today2 ))
            self.conn.commit()
            if (rowcount == 1):
    ...
    def validate_check(self):
        if len(self.t1.get()) < 4  or not(self.t1.get().isdigit()):
            messagebox.showwarning("Validation Check", "Please Enter Valid Year ", parent=self.window)
            return False
        elif  int(self.t1.get())< 2000 or  int(self.t1.get())>self.currentyear:
            messagebox.showwarning("Validation Check", "Please Enter year between 2000 to current year ", parent=self.window)
            return False
            ...
            messagebox.showwarning("Validation Check", "Please Enter Showroom Price more than 1 lakh", parent=self.window)
            return False
